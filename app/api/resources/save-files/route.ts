...
export const POST = withAuth(async (req: NextRequest) => {
  try {
    ...
    const validFiles = files
      .map((fileData, i) => {
        const parsed = ResourceCreateSchema.safeParse(fileData)
        if (!parsed.success) {
          errors.push({ index: i, error: parsed.error.issues })
          return null
        }
        return { ...fileData, created_by: userId, updated_by: userId }
      })
      .filter(Boolean)

    let results: ResourceResponse[] = []
    if (validFiles.length > 0) {
      try {
        results = await resourceModel.insertMany(validFiles, { ordered: false })
      } catch (e: any) {
