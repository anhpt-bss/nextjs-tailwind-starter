![tailwind-nextjs-banner](/public/static/images/app/banner.png)
...
Facing issues? Check the [FAQ page](https://github.com/anhpt-bss/nextjs-tailwind-starter/wiki) and do a search on past issues. Feel free to open a new issue if none has been posted previously.
...
```bash
npx degit 'anhpt-bss/nextjs-tailwind-starter'
```
...
> [!IMPORTANT]
> If deploying with a URL base path, like https://example.org/myblog you need an extra `NEXT_PUBLIC_BASE_PATH` shell-var to the build command:
>
> ```sh
> $ EXPORT=1 UNOPTIMIZED=1 NEXT_PUBLIC_BASE_PATH=/myblog yarn build
> ```
>
> => In your code, `${process.env.NEXT_PUBLIC_BASE_PATH || ''}/robots.txt` will print `"/myblog/robots.txt"` in the `out` build (or only `/robots.txt` if `yarn dev`, ie: on localhost:3000)
